...
library ShieldSystem requires Base

private function Unit_Add_Shield_Func takes nothing returns nothing
    local timer t = GetExpiredTimer()
    local integer id = GetHandleId(t)
    local unit target = LoadUnitHandle(HT, id, 0)
    local real duration = LoadReal(HT, id, 1)
    local real shield_value = LoadReal(HT, id, 2)
...
function Unit_Add_Shield takes unit target, real duration, real shield_value, string eff, string attach_point, real delay returns nothing
    local timer t = CreateTimer()
    local integer id = GetHandleId(t)
    
    call SaveUnitHandle(HT, id, 0, target)
    call SaveReal(HT, id, 1, duration)
    call SaveReal(HT, id, 2, shield_value)
    call SaveStr(HT, id, 3, eff)
    call SaveStr(HT, id, 4, attach_point)
    call TimerStart(t, delay, false, function Unit_Add_Shield_Func)
    
    set t = null
endfunction

function Unit_Add_Shield_Area takes unit u, real x, real y, real size, real duration, real shield_value, string eff, string attach_point, real delay returns nothing
...
private function Unit_Add_Shield_Area_Attached_Func takes nothing returns nothing
    local timer t = GetExpiredTimer()
    local integer id = GetHandleId(t)
    ...
    local real shield_value = LoadReal(HT, id, 5)
    local string eff = LoadStr(HT, id, 6)
    local string attach_point = LoadStr(HT, id, 7)
    local group g = CreateGroup()
    local unit c = null
...
function Unit_Add_Shield_Area_Attached takes unit u, real dist, real angle, real size, real duration, real shield_value, string eff, string attach_point, real delay returns nothing
    local timer t = CreateTimer()
    local integer id = GetHandleId(t)
    ...
    call SaveReal(HT, id, 5, shield_value)
    call SaveStr(HT, id, 6, eff)
    call SaveStr(HT, id, 7, attach_point)
    call TimerStart(t, delay, false, function Unit_Add_Shield_Area_Attached_Func)

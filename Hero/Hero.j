struct Hero
    ...
    public method Stat_Alloc_Increase takes integer property, integer value returns nothing
        local integer add = 0

        if property == AD then
            set add = value - increase_ad
            set increase_ad = value
            call Set_AD( Get_AD() + add/2 )
            
        elseif property == AP then
            set add = value - increase_ap
            set increase_ap = value
            call Set_AP( Get_AP() + add )
            
        elseif property == AS then
            set add = value - increase_as
            set increase_as = value
            call Set_AS( Get_AS() + add/3 )
            
        elseif property == MS then
            set add = value - increase_ms
            set increase_ms = value
            call Set_MS( Get_MS() + add/4 )
            
        elseif property == HP then
            set add = value - increase_hp
            set increase_hp = value
            call Set_HP( Get_HP() + add*15 )
            
        elseif property == MP then
            set add = value - increase_mp
            set increase_mp = value
            call Set_MP( Get_MP() + add*3 )
            
        elseif property == DEF_AD then
            set add = value - increase_def_ad
            set increase_def_ad = value
            call Set_DEF_AD( Get_DEF_AD() + add/2 )
            
        elseif property == DEF_AP then
            set add = value - increase_def_ap
            set increase_def_ap = value
            call Set_DEF_AP( Get_DEF_AP() + add/2 )
            
        elseif property == HP_REGEN then
            set add = value - increase_hp_regen
            set increase_hp_regen = value
            call Set_HP_REGEN( Get_HP_REGEN() + add/2 )
            
        elseif property == MP_REGEN then
            set add = value - increase_mp_regen
            set increase_mp_regen = value
            call Set_MP_REGEN( Get_MP_REGEN() + add/10 )
        else
